...
import { Menu, Copy, LogOut } from 'lucide-react';
...
export default function Header() {
  ...
  const renderNavLink = (link: NavLink) => (
    <Link
      ...
      href={link.href}
      className={cn(
        "relative px-2 lg:px-4 py-2 text-sm lg:text-base font-medium text-gray-100 rounded-md hover:text-purple-600 hover:bg-gray-200 transition-all duration-200 group whitespace-nowrap",
        ...
        <div className="flex items-center justify-between py-4">
          <Link href="/" className="flex items-center space-x-2 flex-shrink-0">
            <Image 
              ...
              width={90} 
              height={90} 
              className="h-16 w-16 md:h-10 md:w-10 object-contain"
            />
            <span className="text-lg md:text-xl font-bold text-purple-600">OPEN MARKET</span>
          </Link>

            <MaxWidthWrapper>
              <nav className="hidden lg:flex items-center space-x-1 flex-grow justify-center max-w-2xl mx-4">
                  <div className="rounded-md backdrop-blur-md bg-slate-400 bg-white/30 px-2 py-1.5 shadow-sm border border-white/50 flex flex-wrap justify-center">
